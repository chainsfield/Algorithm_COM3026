...
int main() {
    ...
    Review review;
    InitReview(&review);
    review.now = &review.rquestion;
    Field Arithmetic, Fractional, ProblemSolve, Time, Unit;
    Fields myFields;
    myFields.Arithmetic = &Arithmetic;
    myFields.Time = &Time;
    myFields.Fractional = &Fractional;
    myFields.ProblemSolve = &ProblemSolve;
    myFields.Unit = &Unit;
    GQue useranswer;
    useranswer.head = 0;
    useranswer.tail = 0;
    ...
    for (int i = 0; i <= 99; i++)
        fgets(newProsol.Problem[i], 300, newfile);
    newfile = fopen("All_A.txt", "r");
    for (int i = 0; i <= 99; i++)
        fgets(newProsol.Solution[i], 300, newfile);
...
int DisplayMainMenu() {
    ...
    printf("NAME >> ");
    char name[10];
    gets(name); // 처음에 이름 입력받고 인사
    gotoxy(50, 17);
    printf("%s♥", name);

    int menu;
    gotoxy(40, 26);
    printf("초등학생용 맞춤형 학습지");
    gotoxy(44, 27);
    printf("1. 테스트 보기");
    gotoxy(44, 28);
    printf("2. 오답노트 확인");
...
void DoTest(Test *test, Review *review, Fields *fields, GQue *useranswer, int *testscore, int testcount) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (int j = 0; j < len; j++) {
            ...
            if (curInfo.dwCursorPosition.X % 70 == 69 && l != 1) {
                printf("\n");
                gotoxy(10, k);
                k++;
            }
...
void ShowReview(Review *review) {
    ...
    for (int i = 0; i < idx; i++) {
        ...
        gotoxy(13, 20);
        printf("▶ ");
        scanf("%s", review_answer);
        len = strlen(review_answer);
        for (int j = 0; j < len; j++) {
            if (review_answer[j] == ' ')
                review_answer[j] = review_answer[j + 1];
        }
