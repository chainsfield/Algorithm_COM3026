...
SCREEN_WIDTH = 600  # 게임판 가로
SCREEN_HEIGHT = 480  # 게임판 세로

S_HEIGHT = SCREEN_HEIGHT - 50
...
effectOpacity = [0, 0, 0]  # 내려오는 각 막대기의 투명도
score = 0  # score 변수값
combo = 0  # combo 변수값
showScore = 0


# 믹서 초기화 인수 설정 pre_init(frequency, size, channels, buffersize)  소리 옵
pygame.mixer.pre_init(44100, -16, 2, 2048)
pygame.mixer.init()  # 믹서 설정
pygame.init()
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), HWSURFACE | DOUBLEBUF)  # 게임판 생성
...
weRockNote.close()
showChaebo.append(timeArr)
# deepcopy 사용, 원본값이 변경되더라도 이전에 받은 값 유지
correctChaebo = copy.deepcopy(showChaebo)
...
class Note(pygame.sprite.Sprite):
    ...
    def update(note):
        ...
        note.rect.y = SCREEN_HEIGHT - ((note.s_Time - getRuntime()) / (SHOWN_TIME / SPEED_RATE)) * S_HEIGHT - (SCREEN_HEIGHT - S_HEIGHT)
...
def printGame():
    try:
        ...
        backpic = pygame.transform.scale(backpic, (SCREEN_WIDTH, SCREEN_HEIGHT))
        screen.blit(backpic, backpic.get_rect())

        allNoteList.update()
        allNoteList.draw(screen)

        pygame.draw.line(screen, WHITE, [50 + fallBar.get_rect().width * 0, 0], [50 + fallBar.get_rect().width * 0, SCREEN_HEIGHT], 2)
        for i in range(1, 3):
            pygame.draw.line(screen, WHITE, [50 + fallBar.get_rect().width * i, 0], [50 + fallBar.get_rect().width * i, SCREEN_HEIGHT], 2)
        pygame.draw.line(screen, WHITE, [50 + fallBar.get_rect().width * 3, 0], [50 + fallBar.get_rect().width * 3, SCREEN_HEIGHT], 2)

        pygame.draw.line(screen, WHITE, [50, S_HEIGHT], [50 + fallBar.get_rect().width * 3, S_HEIGHT], 2)
        for i in range(0, 3):
            gameFont = pygame.font.Font("dabang.ttf", 36)
            keyRect = pygame.Rect(50 + fallBar.get_rect().width * i + 2, S_HEIGHT + 2, fallBar.get_rect().width - 2, SCREEN_HEIGHT - S_HEIGHT - 2)
            if i == 0:
